...
class TestRecentScrobblesCli:
    ...
    def test_first_run_initializes_state(self, tmp_path, capsys):
        """Test first run with no state file initializes tracking and exits without fetching."""
        state_file = tmp_path / "nonexistent.json"
        user = self._make_user()

        with patch("spotfm.lastfm.LASTFM_STATE_FILE", state_file):
            recent_scrobbles(user, limit=None, scrobbles_minimum=0, period=90, period_minimum=None, interactive=False)
    ...
    def test_since_last_time_no_new_scrobbles(self, tmp_path, capsys):
        """Test state-based run when count has not changed."""
        state_file = tmp_path / "lastfm_state.json"
    ...
    def test_since_last_time_fetches_diff(self, tmp_path):
        """Test state-based run computes correct limit from diff."""
        state_file = tmp_path / "lastfm_state.json"
    ...
    def test_since_last_time_updates_state(self, tmp_path):
        """Test state-based run updates the state file after fetching."""
        state_file = tmp_path / "lastfm_state.json"
        save_lastfm_state(135, state_file=state_file)
        user = self._make_user(playcount=173)
    ...
    def test_explicit_limit_overrides_state(self, tmp_path):
        """Test that explicit --limit fetches that many scrobbles regardless of state."""
        state_file = tmp_path / "lastfm_state.json"
        save_lastfm_state(171, state_file=state_file)  # only 2 new scrobbles
        user = self._make_user(playcount=173)

        with patch("spotfm.lastfm.LASTFM_STATE_FILE", state_file):
            recent_scrobbles(user, limit=50, scrobbles_minimum=0, period=90, period_minimum=None, interactive=False)

        call_args = user.get_recent_tracks_scrobbles.call_args
        assert call_args[0][0] == 50  # fetches 50, not 2

    def test_since_last_time_prints_diff_info(self, tmp_path, capsys):
        """Test state-based run prints informational message with counts."""
        state_file = tmp_path / "lastfm_state.json"
        save_lastfm_state(135, state_file=state_file)
        user = self._make_user(playcount=173)
